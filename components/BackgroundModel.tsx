...
function AudioWaveform({ isLight }: { isLight: boolean }) {
  ...
  // Track elapsed time via delta accumulation — avoids THREE.Clock deprecation warning
  ...
  useFrame((_, delta) => {
    ...
    const time = elapsedRef.current * 1.5;
    ...
    if (meshRef.current) {
      ...
      meshRef.current.rotation.z = elapsedRef.current * 0.02;
    }
  });
