...
export default function ProductPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-32 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16">
        ...
        <div className="w-full flex flex-col justify-start">
          <motion.div
            ...
            className="relative w-full aspect-square overflow-hidden glass-card rounded-3xl flex items-center justify-center p-8"
          >
            {/* Inner ambient glow */}
            ...
            {/* Stock badge overlay */}
            <div className="absolute top-8 right-8 flex items-center gap-2 bg-black/40 backdrop-blur-md border border-white/10 px-4 py-2 rounded-full shadow-glass z-10">
              <motion.span
        ...
        <div className="w-full flex flex-col justify-start">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 max-w-xl">
              <div className="flex items-center justify-between border border-white/10 bg-black/20 h-14 w-full sm:w-36 shrink-0 rounded-full px-2">
            ...
            {/* Tabs */}
            <div className="mt-4 border border-aluminum/20 bg-[#111614] max-w-xl rounded-lg overflow-hidden">
              <div className="flex border-b border-aluminum/20 bg-[#0a0d0c]" role="tablist">
                {([{ id: 'specs', label: 'SPECS' }, { id: 'box', label: "CONTENTS" }, { id: 'compat', label: 'COMPAT' }] as const).map((tab) => (
                  <button
                    ...
                    className={`flex-1 font-mono text-[9px] px-2 md:px-4 py-3 transition-colors uppercase ${
                  ...
                  <div className="space-y-2">
                    {Object.entries(product.specifications).map(([key, value]) => (
                      <div key={key} className="flex justify-between items-end border-b border-aluminum/10 pb-1">
                        ...
                        <span className="font-mono text-[10px] text-cable-white uppercase text-right max-w-[60%] truncate" title={value}>{value}</span>
                      </div>
                    ))}
                  </div>
                )}
      ...
      {related.length > 0 && (
        <div className="mt-24 pt-12 border-t border-aluminum/10">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 gap-4">
            <h2 className="font-mono text-xl md:text-2xl font-bold text-cable-white uppercase">
              [ RELATED_SIGNALS ]
            </h2>
            <Link
              href={`/shop?category=${catSlug}`}
              className="font-mono text-[10px] text-signal hover:underline transition-colors uppercase border border-signal/30 px-4 py-2 rounded"
            >
              EXPAND_VIEW →
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {related.map((p, i) => (
              <TechnicalProductCard key={p.id} product={p} index={i} />
            ))}
          </div>
        </div>
      )}
